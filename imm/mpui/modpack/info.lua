...
function IUI:renderDesc()
	local _, descs = G.LANG.font.FONT:getWrap(self.mp.description, G.TILESCALE * G.TILESIZE * 20 * self.descWidth)
	local descElm = {}
	for i,v in ipairs(descs) do
		if i > self.descLines then break end
...
function IUI:renderInputContainer(prop)
	return ui.R{ padding = 0.1, self:renderInputField(prop) }
end
...
--- @protected
function IUI:renderButton(label, button)
	return UIBox_button({
		colour = self.ses.inputColors,
		label = label,
		minh = self.ses.fontScale * 1.5,
		col = true,
		button = button,
		ref_table = self
	})
...
function IUI:renderCols()
	--- @type balatro.UIElement.Definition[]
	return {
		ui.R{ align = 'cm', nodes = self:renderHeader() },
		ui.R{ align = 'cm', padding = 0.10, self:renderDesc() },
		ui.R{ align = 'cm', padding = 0.05, ui.TRS("Drag and drop an image fie to set icon.", self.ses.fontScale, {0.8, 0.8, 0.8, 1}) },
		ui.R{ align = 'cm', padding = 0.05, nodes = ui.gapList('C', 0.1, self:renderEditOpts()) }
	}
end
